...
export interface VirtualQueries<T, W> {
  ...
  get<K extends keyof T, A extends string, N>(params: W | null, columns: (Alias<T, A, N> | K)[] | (keyof T)[]): Promise<(Pick<T, K> & Record<A, (N extends TableProperty ? JsonValue : N)>) | undefined>;
  get<K extends keyof T>(params: W | null, column: K): Promise<T[K] | undefined>;
  get<N>(params: W | null, column: (selector: TableObject<T>) => N): Promise<(N extends TableProperty ? JsonValue : N) | undefined>;
  ...
  many<K extends keyof T, A extends string, N>(params: W | null, columns: (Alias<T, A, N> | K)[] | (keyof T)[]): Promise<Array<(Pick<T, K> & Record<A, (N extends TableProperty ? JsonValue : N)>)>>;
  many<K extends keyof T>(params: W | null, column: K): Promise<Array<T[K]>>;
  many<N>(params: W | null, column: (selector: TableObject<T>) => N): Promise<Array<(N extends TableProperty ? JsonValue : N)>>;
  query<K extends keyof T, A extends string, N>(query: VirtualQueryObject<W, A, K, T, N>): Promise<Array<(Pick<T, K> & Record<A, (N extends TableProperty ? JsonValue : N)>)>>;
  query<K extends keyof T>(query: VirtualQueryValue<W, K, T>): Promise<Array<T[K]>>;
  query(query: VirtualQuery<W, T>): Promise<Array<T>>; 
  query<N>(query: VirtualQuerySelector<W, T, N>): Promise<Array<(N extends TableProperty ? JsonValue : N)>>;
...
export interface Queries<T, I, W, R> {
  ...
  many<K extends keyof T, A extends string, N>(params: W | null, columns: (Alias<K, A, N> | K)[] | (keyof T)[]): Promise<Array<(Pick<T, K> & Pick<{ [key: string]: (N extends TableProperty ? JsonValue : N) }, A>)>>;
  many<K extends keyof T>(params: W | null, column: K): Promise<Array<T[K]>>;
  many<N>(params: W | null, column: (selector: TableObject<T>) => N): Promise<Array<N extends TableProperty ? JsonValue : N>>;
